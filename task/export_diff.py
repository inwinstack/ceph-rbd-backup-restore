...
class ExportDiffTask(Task):
    def __init__(self, conf_file, keyring_file, cluster_name, pool_name, rbd_name,
                 snap_name=None, from_snap=None, dest_path=None, dest_file=None,
                 export_datetime_fmt=None):
        super(ExportDiffTask, self).__init__()
        ...
        self.dest_filepath = ""
        self.export_type = 'diff'
        self.monitor = True
        self.create_datetime = None

    def __str__(self):
        if self.snap_name == None:
            return "diff_export_%s/%s_from_%s" %(self.pool_name,
                                                 self.rbd_name,
                                                 self.from_snap)
        else:
            return "diff_export_%s/%s@%s_from_%s" %(self.pool_name,
                                                    self.rbd_name,
                                                    self.snap_name,
                                                    self.from_snap)
    ...
    def get_command(self):
        try:
            ...
            if self.dest_file == None:
                if self.snap_name == None:
                    self.dest_file = "%s_from_%s" % (self.create_datetime,
                                                     self.from_snap)
                else:
                    self.dest_file = "%s_from_%s" % (self.snap_name,
                                                     self.from_snap)

            self.dest_filepath = os.path.join(self.dest_path,
                                              self.dest_file)

            self.command = "rbd export-diff --no-progress " \
                           "--conf %s --keyring %s --cluster %s --pool %s " \
                           "%s %s %s" % (self.conf_file,
                                         self.keyring_file,
                                         self.cluster_name,
                                         self.pool_name,
                                         rbd_name,
